...
template <typename T> T std::atomic<T>::operator=(T val) noexcept {
	__atomic_store_n(&value, val, __ATOMIC_SEQ_CST);
	return val;
}
template <typename T> T std::atomic<T>::operator=(T val) volatile noexcept {
	__atomic_store_n(&value, val, __ATOMIC_SEQ_CST);
	return val;
}
...
/* Read the value */
template <typename T> T std::atomic<T>::load(memory_order sync) noexcept {
...
template <typename T> std::atomic<T>::operator T() const noexcept {
	return __atomic_load_n(&value, __ATOMIC_SEQ_CST);
}
template <typename T> std::atomic<T>::operator T() const volatile noexcept {
	return __atomic_load_n(&value, __ATOMIC_SEQ_CST);
}
...
/* These next four may return false without modifying expected even if it equals
 * val (may increase preformance) */
template <typename T>
bool std::atomic<T>::compare_exchange_weak(T &expected, T val,
                                           memory_order sync) noexcept {
	return __atomic_compare_exchange_n(&value, &expected, val, true, sync,
	                                   sync);
}
template <typename T>
bool std::atomic<T>::compare_exchange_weak(
	T &expected, T val, memory_order sync) volatile noexcept {
	return __atomic_compare_exchange_n(&value, &expected, val, true, sync,
	                                   sync);
}
template <typename T>
bool std::atomic<T>::compare_exchange_weak(T &expected, T val,
	...
	return __atomic_compare_exchange_n(&value, &expected, val, true, success,
	                                   failure);
}
template <typename T>
bool std::atomic<T>::compare_exchange_weak(
	T &expected, T val, memory_order success,
...
template <typename T>
bool std::atomic<T>::compare_exchange_strong(T &expected, T val,
                                             memory_order sync) noexcept {
	return __atomic_compare_exchange_n(&value, &expected, val, false, sync,
	                                   sync);
}
template <typename T>
...
bool std::atomic<T>::compare_exchange_strong(T &expected, T val,
                                             memory_order success,
                                             memory_order failure) noexcept {
	return __atomic_compare_exchange_n(&value, &expected, val, false, success,
	                                   failure);
}
template <typename T>
bool std::atomic<T>::compare_exchange_strong(
...
template <typename T> bool std::atomic<T>::is_lock_free() const noexcept {
