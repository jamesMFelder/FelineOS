...
static void split_header(Header* &current_header, size_t split) {
    ...
    // This allows a 1-byte other allocation which feels wrong but keeps this generic and symmetric (split can be 1)
    if (current_header->len < split+sizeof(Header)) {
		std::abort();
    }
    Header *new_header = reinterpret_cast<Header*>(reinterpret_cast<uintptr_t>(current_header)+split+sizeof(Header));
...
[[nodiscard]] static Header* allocate_more_mem(size_t needed) {
	Header *hdr;
#ifdef __is_libk
	auto result = get_mem(reinterpret_cast<void**>(&hdr), round_to_multiple_of(needed, DEFAULT_MEMRESERVE_SIZE));
...
void free(void* ptr) {
	...
	if (hdr->next && !hdr->next->in_use && !hdr->next->start_of_allocation && \
			hdr->next == reinterpret_cast<Header*>(reinterpret_cast<uintptr_t>(hdr)+hdr->len+sizeof(Header))) {
		hdr->len += hdr->next->len+sizeof(Header);
		hdr->next = hdr->next->next;
	}
	if (!hdr->start_of_allocation && hdr->prev && hdr->prev->in_use == false && \
			hdr == reinterpret_cast<Header*>(reinterpret_cast<uintptr_t>(hdr->prev)+hdr->prev->len+sizeof(Header))) {
	...
	while (hdr->start_of_allocation && hdr->len >= DEFAULT_MEMRESERVE_SIZE-sizeof(Header)) {
