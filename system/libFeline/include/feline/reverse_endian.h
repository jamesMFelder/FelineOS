...
constexpr inline uint16_t reverse_endian(const uint16_t value) {
	uint16_t reversed = 0;
...
constexpr inline uint32_t reverse_endian(const uint32_t value) {
	uint32_t reversed = 0;
...
constexpr inline uint64_t reverse_endian(const uint64_t value) {
	uint64_t reversed = 0;
...
concept EndianReverseable = requires(T a) {
	{ reverse_endian(a) } -> std::same_as<std::remove_const_t<T>>;
};
