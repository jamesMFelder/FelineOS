...
int framebuffer::putRect(uint16_t x, uint16_t y, uint16_t width,
                         uint16_t height, pixel_t p) {
...
int framebuffer::putPixel_bgr(uint16_t x, uint16_t y, pixel_bgr_t p) {
	...
	if (inval != 0) {
		...
		/* If the padding is bad (this shouldn't be possible, because this is a
		 * private function) */
		if ((inval & 0b10) != 0) {
			return -2;
		}
...
int framebuffer::putRect_bgr(uint16_t x, uint16_t y, uint16_t width,
                             uint16_t height, pixel_bgr_t p) {
	/* Check everything is okay */
...
unsigned int framebuffer::checkParms(uint16_t maxRight, uint16_t maxDown,
                                     pixel_bgr_t p) {
	/* No errors to start */
